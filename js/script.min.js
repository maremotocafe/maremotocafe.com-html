...
window.onscroll=toggleScrollToTop;toggleScrollToTop();const observer=lozad();observer.observe();$(window).on('hashchange',function(){if(location.href.indexOf("#item")<0){$.magnificPopup.close();}});function onOpenPopup(){location.href=location.href.split('#')[0]+"#item";}
function onClosePopup(){if(location.hash){history.go(-1);}}
$('.open-popup-inline').magnificPopup({type:'inline',midClick:true,fixedContentPos:false,fixedBgPos:true,callbacks:{open:onOpenPopup,close:onClosePopup,}});$('.open-popup-standalone').magnificPopup({type:'image',midClick:true,closeOnContentClick:true,fixedContentPos:false,fixedBgPos:true,callbacks:{open:onOpenPopup,close:onClosePopup,}});const containerEl=document.querySelector('.shuffle-wrapper');var myShuffle=new window.Shuffle(containerEl,{itemSelector:'.shuffle-item',buffer:1,useTransforms:false});Array.from(document.getElementsByClassName('lozad')).forEach(img=>img.addEventListener('load',()=>myShuffle.layout()));if(containerEl){const allInputs=document.querySelectorAll('input[name="shuffle-filter"]');const toggableFilters=document.querySelectorAll('.menu-filter:not([filter-level="1"])');const noItemsMsg=document.getElementById('no-items-msg');const loadMoreBtn=document.getElementById('load-more-btn');const INITIAL_NUM_ITEMS=parseInt(loadMoreBtn.getAttribute('load-max'));const INCREMENT_LOAD=parseInt(loadMoreBtn.getAttribute('load-increment'));let numItems=INITIAL_NUM_ITEMS;let prevFilter='all';function getBar(filter){return filter.children[1];}
function barContains(bar,value){return Array.from(bar.children).some(i=>i.children[0].getAttribute('filter-value')===value);}
function resetFilter(filter){function deactivate(el){el.classList.remove('active');el.classList.checked=false;}
function activate(el){el.classList.add('active');el.classList.checked=true;}
let bar=getBar(filter);deactivate(Array.from(bar.children).find(i=>i.classList.contains('active')));activate(bar.children[0]);}
function hideFilter(filter){const filterJQ=$(filter);if(filterJQ.is(':hidden')){return}
resetFilter(filter);filterJQ.css('opacity',1);filterJQ.addClass('d-none');filterJQ.animate({opacity:0},400);}
function showFilter(filter){const filterJQ=$(filter);if(!filterJQ.is(':hidden')){return}
...
Array.from(allInputs).forEach(item=>item.addEventListener('click',e=>{const curInput=e.currentTarget;const curFilter=curInput.getAttribute('filter-value');const curBar=curInput.parentNode.parentNode;const curParent=curBar.parentNode.getAttribute('filter-parent');toggableFilters.forEach(filter=>{const filterParent=filter.getAttribute('filter-parent');let bar=getBar(filter);if(bar===curBar||barContains(bar,curParent)||filterParent===curFilter)
{showFilter(filter);if(filterParent===curFilter){resetFilter(filter);}}else{hideFilter(filter);}});filterItems(curFilter);}));loadMoreBtn.addEventListener('click',e=>{numItems+=INCREMENT_LOAD;filterItems(prevFilter);});}
