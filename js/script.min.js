...
return false;}
function resetBar(bar){for(let i=0;i<bar.children.length;i++){if(bar.children[i].classList.contains('active')){bar.children[i].classList.remove('active');bar.children[i].classList.checked=false;break;}}
bar.children[0].classList.add('active');bar.children[0].classList.checked=true;}
function hideBar(bar){const bar_jq=$(bar);if(bar_jq.is(':hidden')){return}
resetBar(bar);bar_jq.css('opacity',1);bar_jq.removeClass('d-flex');bar_jq.addClass('d-none');bar_jq.animate({opacity:0},400);}
...
myShuffle.filter(input.value);bars.forEach((bar)=>{const bar_parent=bar.getAttribute('parent');if(bar===input_bar||contains(bar,input_bar.getAttribute('parent'))||bar_parent===input.value){showBar(bar);if(bar_parent===input.value){resetBar(bar);}}else{hideBar(bar);}});});}
