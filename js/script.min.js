'use strict';window.addEventListener('load',function(e){function browserIncompatible(){if(document.querySelectorAll('.thisdoesntexist').forEach==='undefined'){return true;}}
if(browserIncompatible()){document.querySelector('.old-browser').style.display='flex';}
const minScroll=1000;const scrollBtn=document.getElementById('scroll-to-top');function toggleScrollToTop(){if(document.documentElement.scrollTop>minScroll||window.pageYOffset>minScroll||document.body.scrollTop>minScroll){scrollBtn.style.display="block";}else{scrollBtn.style.display="none";}}
...
message.style.borderColor=okColor;let uri='mailto:yesus19@hotmail.es?subject='
+encodeURIComponent(subject)+'&body='
+encodeURIComponent(message);window.open(uri,'_blank');helpMsg.style.display='block'});});